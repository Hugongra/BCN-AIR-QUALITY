...
# Full dataset after joinning all sub datasets together
...
# Pivot table to have pollution index in rows instead of columns
pivot_table <- pivot_longer(df, cols = 10:33, names_to = "Hours", values_to = "Pollution")

# Drop missing values
pivot_table <- na.omit(pivot_table)

# Transform hour column to integer
pivot_table$Hours <- as.numeric(sub("H", "", pivot_table$Hours))

# check columns classes
sapply(pivot_table, class)

# check unique values of hours columns
unique(pivot_table$Hours)

# combine all year month day and hours to one datetime column
date <- pivot_table %>%
    select(ANY, MES, DIA, Hours) %>%
    mutate(date = make_datetime(ANY, MES, DIA, Hours)) %>%
    select(date)

# Add datetime column to our dataframe
pivot_table <- cbind(pivot_table, date)

######################################################################

# Here it is imposible to see the peeks
ggplot(pivot_table %>% filter(ESTACIO == 4), aes(x = date, y = Pollution)) +
    geom_line(aes(color = CODI_CONTAMINANT), linewidth = 1) +
    theme_minimal()

# adding avg pollution column
daily_df <- pivot_table %>%
    group_by(ESTACIO, CODI_CONTAMINANT, ANY, MES, DIA) %>%
    summarise("avg_daily_pollution" = mean(Pollution))
view(daily_df)

# creating new colmn
date_daily <- daily_df %>%
    select(ANY, MES, DIA) %>%
    mutate(date_daily = make_datetime(ANY, MES, DIA))
view(date_daily)

# Add datetime column to our dataframe
daily_df$date_daily <- date_daily$date_daily
view(daily_df$date_daily)

# Here is the better graph which makes the peaks more visible
ggplot(daily_df %>% filter(ESTACIO == 4), aes(x = date_daily, y = avg_daily_pollution)) +
    geom_line(aes(color = CODI_CONTAMINANT), linewidth = 1) +
    theme_minimal()
